...
def setup_logging(
    log_level: Optional[str] = None, log_file: Optional[str] = None, json_format: bool = False
) -> logging.Logger:
    """
    ...
    if log_file_path:
        try:
            ...
            warnings.warn(
                f"Не удалось создать файл лога {log_file_path}: {e}. Используется только консольное логирование."
            )
