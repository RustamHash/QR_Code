...
class ProcessingHistory(Base):
    ...
    processing_type = Column(SQLEnum(ProcessingType), nullable=False, index=True)
    source_name = Column(String(500))  # Имя файла или "text"
    qr_codes_count = Column(Integer, nullable=False, default=0)
    status = Column(
        SQLEnum(ProcessingStatus), nullable=False, default=ProcessingStatus.SUCCESS, index=True
    )
...
Index(
    "idx_processing_history_user_type", ProcessingHistory.user_id, ProcessingHistory.processing_type
)
